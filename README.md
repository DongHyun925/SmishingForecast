...
1. **윤리적 및 법적 책임**: 본 프로젝트는 보안 연구 및 교육적 목적으로만 설계되었습니다. 수집된 뉴스 데이터의 저작권은 각 언론사 및 포털에 있으며, 본 코드를 이용한 무단 데이터 대량 수집 및 상업적 활용에 따른 모든 책임은 사용자에게 있습니다.
2. **공식 채널 활용**: 우리 시스템의 크롤링 기능은 Google RSS 및 Naver Search API 등 원격지에서 공식적으로 허용한 통로를 우선 활용합니다.
3. **API 비용**: OpenAI API 사용량에 따라 개인용 API의 경우 비용이 발생할 수 있습니다.
4. **데이터 보안**: `.env` 파일에 포함된 API 키 등 민감 정보는 절대 공개 저장소에 업로드하지 마세요. (본 프로젝트는 `.gitignore`를 통해 이를 보호하고 있습니다.)
